...
# Install 7zip static binary
mkdir /tmp/7z
cd /tmp/7z
curl -f -L -k -o 7z-linux-x64.tar.xz "https://github.com/ip7z/7zip/releases/download/24.09/7z2409-linux-x64.tar.xz"
tar xf 7z-linux-x64.tar.xz
mv 7zzs /usr/bin/
[ -f License.txt ] && mv License.txt /usr/share/ || true
cd - 1> /dev/null
rm -rf /tmp/7z
...
if grep -q '^CHARSET=' /etc/sysconfig/oracle-xe-21c.conf; then
  sed -i "s/^CHARSET=.*/CHARSET=WE8MSWIN1252/g" /etc/sysconfig/oracle-xe-21c.conf
else
  echo "CHARSET=WE8MSWIN1252" >> /etc/sysconfig/oracle-xe-21c.conf
fi
